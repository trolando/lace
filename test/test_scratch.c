...
void basic_alloc_task_CALL(lace_worker *lw)
{
...
static void test_basic_alloc(void) { basic_alloc_task(); }

/* --- 2. Alignment ------------------------------------------------------- */

TASK(void, alignment_task)

void alignment_task_CALL(lace_worker* lw)
{
    void *mark = lace_scratch_mark(lw);
...
static void test_alignment(void) { alignment_task(); }
...
void interleaved_alloc_task_CALL(lace_worker* lw)
{
    void *mark = lace_scratch_mark(lw);

    uint8_t *bytes = LACE_SCRATCH_ARRAY(lw, uint8_t, 3);
    EXPECT(bytes != NULL);
    EXPECT_EQ((uintptr_t)bytes & 15u, 0u);
    bytes[0] = 0xAA; bytes[1] = 0xBB; bytes[2] = 0xCC;

    uint32_t *vars = LACE_SCRATCH_ARRAY(lw, uint32_t, 5);
    EXPECT(vars != NULL);
    ...
    uint64_t *bdds = LACE_SCRATCH_ARRAY(lw, uint64_t, 7);
    EXPECT(bdds != NULL);
...
static void test_interleaved_alloc(void) { interleaved_alloc_task(); }

/* --- 3. mark/reset roundtrip ------------------------------------------- */

TASK(void, markreset_task)

void markreset_task_CALL(lace_worker* lw)
{
    ...
    lace_scratch_reset(lw, mark);
    EXPECT_EQ(lw->scratch_top, before);
}

static void test_markreset(void) { markreset_task(); }

/* --- 4. Recursive task body with explicit mark/reset -------------------- */

TASK(int, recursive_task, int, depth)

int recursive_task_CALL(lace_worker* lw, int depth)
{
    if (depth == 0) return 0;
    void *mark = lace_scratch_mark(lw);
    ...
    int sub = recursive_task_CALL(lw, depth - 1);
...
int sum_task_CALL(lace_worker* lw, int n)
{
    if (n <= 1) {
        void *mark = lace_scratch_mark(lw);
...
void burn_scratch_CALL(lace_worker* lw, int mb)
{
    void *mark = lace_scratch_mark(lw);
...
void leaky_task_CALL(lace_worker* lw)
{
    /* Mark, alloc, return WITHOUT reset. */
    (void)lace_scratch_mark(lw);
    char *buf = (char *)lace_scratch_alloc(lw, 4096);
    EXPECT(buf != NULL);
...
int no_scratch_fib_CALL(lace_worker* lw, int n)
{
    if (n < 2) return n;
