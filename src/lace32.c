...
/**
 * Per-worker scratch arena reservation size in bytes (default 1 GiB).
 *
 ...
 */
static size_t scratchsize = (size_t)1 << 30;
 ...
 *
 * The arena is task-scoped: the generated NAME_CALL wrapper around
 * each TASK body saves and restores scratch_top, so all allocations
 * inside the body are reclaimed automatically when it returns.
 * ======================================================================== */
...
lace_scratch_grow(lace_worker *lw, size_t aligned_size)
{
    ...
    if (LACE_UNLIKELY(new_top > lw->scratch_reserve_end)) {
        fprintf(stderr,
                ...
                "Call lace_set_scratchsize() with a larger value before lace_start().\n",
                (unsigned)lw->worker,
...
void lace_steal_loop_IMPL(lace_worker* lw, atomic_int* quit)
{
    // Determine who I am
 ...
 */
void
lace_set_scratchsize(size_t size)
{
    scratchsize = size;
}
...
void
lace_together_root_IMPL(lace_worker* lw, lace_task* t, atomic_int* finished)
{
    // run the root task
    t->f(lw, t);
...
void
lace_wrap_together_IMPL(lace_worker* worker, lace_task* task)
{
...
void
lace_newframe_root_IMPL(lace_worker *lw, lace_task* t, atomic_int *done)
{
    t->f(lw, t);
    atomic_thread_fence(memory_order_release); // no StoreStore reordering
...
void
lace_wrap_newframe_IMPL(lace_worker* worker, lace_task* task)
{
