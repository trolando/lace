...
#   - README.md        (FetchContent example — no substitution in Markdown)
#
# The following derive the version automatically and need NO manual edits:
#   - src/lace.sh      (reads CMakeLists.txt → regenerate headers with gen.sh)
#   - docs/*.rst       (Sphinx reads CMakeLists.txt via conf.py |version| / |tag|)
#   - lace_config.h    (generated by CMake configure_file)
...
# 1. CMakeLists.txt — update VERSION inside the project() block only
sedi '/^project(/,/)/ s/\(VERSION \)[0-9]*\.[0-9]*\.[0-9]*/\1'"${NEW}"'/' "$REPO_DIR/CMakeLists.txt"

# 2. README.md — update FetchContent version and tag references
